...
half MetallicSetup_Reflectivity(float2 uv, float4 atlas)
{
    return 1.0h - OneMinusReflectivityFromMetallic(MetallicSimple(uv, atlas));
}

half SpecularSetup_Reflectivity(float2 uv, float4 atlas)
{
    return SpecularStrength(SpecularColor(uv, atlas));
}

half RoughnessSetup_Reflectivity(float2 uv, float4 atlas)
{
    return MetallicSetup_Reflectivity(uv, atlas);
}
...
{
    half alpha = Alpha(i.tex.xy, i.atlas, i.color);
    #if defined(_ALPHATEST_ON)
        clip (alpha - Cutoff(i.tex.xy, i.atlas));
    #endif

    FragmentCommonData s = UNITY_SETUP_BRDF_INPUT (i.tex, i.atlas, i.color);
    ...
    #ifdef _NORMALMAP
        s.tangentSpaceNormal =  NormalInTangentSpace(i.tex, i.atlas);
    #else
...
half PerVertexGrazingTerm(VertexOutputBaseSimple i, FragmentCommonData s)
{
    ...
    #else
        half eyeVecw = UNIFORM_REFLECTIVITY(i.tex.xy, i.atlas); // grazing term
        return saturate(GlossinessSimple(i.tex.xy, i.atlas) + eyeVecw);
    #endif
}
...
FragmentCommonData FragmentSetupSimpleAdd(VertexOutputForwardAddSimple i)
{
    half alpha = Alpha(i.tex.xy, i.atlas, i.color);
    #if defined(_ALPHATEST_ON)
        clip (alpha - Cutoff(i.tex, i.atlas));
    #endif
